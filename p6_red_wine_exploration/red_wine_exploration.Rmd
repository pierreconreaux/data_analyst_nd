...
The graph depicts ***the distribution of chloride.***
...
This data set contains ***1,599 red wines with 12 columns.***

***11 numeric variables*** on the chemical properties of the wine and ***1 quality variable.***
...
We did not have to create a new variable for the moment.
...
res<-cor(wine, method = c("spearman"))
```
...
To ensure a better visibility, we create ***rating, a discrete variable***, to gather quality by levels.  

```{r echo=FALSE, rating}
rating <- cut(wine$quality, breaks = c(3, 4, 6, 8), 
